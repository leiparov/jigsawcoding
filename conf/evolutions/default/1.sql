...
alter table grupo_experto add constraint fk_grupo_experto_docente_2 foreign key (docente_dni) references usuario (dni) on delete restrict on update restrict;
create index ix_grupo_experto_docente_2 on grupo_experto (docente_dni);
alter table par_grupo_experto_problema add constraint fk_par_grupo_experto_problema_grupoExperto_3 foreign key (grupo_experto_grupo_experto_id) references grupo_experto (grupo_experto_id) on delete restrict on update restrict;
create index ix_par_grupo_experto_problema_grupoExperto_3 on par_grupo_experto_problema (grupo_experto_grupo_experto_id);
alter table par_grupo_experto_problema add constraint fk_par_grupo_experto_problema_problema_4 foreign key (problema_problema_id) references problema (problema_id) on delete restrict on update restrict;
create index ix_par_grupo_experto_problema_problema_4 on par_grupo_experto_problema (problema_problema_id);
alter table par_grupo_experto_problema add constraint fk_par_grupo_experto_problema_sesionJigsaw_5 foreign key (sesion_jigsaw_id) references sesion_jigsaw (id) on delete restrict on update restrict;
create index ix_par_grupo_experto_problema_sesionJigsaw_5 on par_grupo_experto_problema (sesion_jigsaw_id);
...
alter table problema_examen add constraint fk_problema_examen_problema_8 foreign key (problema_problema_id) references problema (problema_id) on delete restrict on update restrict;
create index ix_problema_examen_problema_8 on problema_examen (problema_problema_id);
alter table sesion_jigsaw add constraint fk_sesion_jigsaw_docente_9 foreign key (docente_dni) references usuario (dni) on delete restrict on update restrict;
create index ix_sesion_jigsaw_docente_9 on sesion_jigsaw (docente_dni);
alter table tema add constraint fk_tema_curso_10 foreign key (curso_codigo) references curso (codigo) on delete restrict on update restrict;
create index ix_tema_curso_10 on tema (curso_codigo);

...
public class ClockController extends Controller {
	...
	public static Result clockReunionExpertos(Integer id) {
	...
	public static Result clockReunionJigsaw(Integer id) {

		SesionJigsaw s = sesionJigsawService.obtenerSesionJigsaw(id);
		Date inicioRJ = s.getInicioReunionJigsaw();
		int duracionRJ = s.getDuracionReunionJigsawEnSegundos();

		Calendar cal = Calendar.getInstance();
		cal.setTime(inicioRJ);
		cal.add(Calendar.SECOND, duracionRJ);
		limite = new Date(cal.getTimeInMillis());
	...
	public static Result clockExamen(Integer id) {

		SesionJigsaw s = sesionJigsawService.obtenerSesionJigsaw(id);
		Date inicio = s.getTiempoAperturaExamen();
		int duracion = s.getDuracionExamenEnSegundos();

		Calendar cal = Calendar.getInstance();
		cal.setTime(inicio);
		cal.add(Calendar.SECOND, duracion);
		limite = new Date(cal.getTimeInMillis());
	...
	public static class Clock extends UntypedActor {
		...
		static {
			Akka.system()
					...
					.schedule(Duration.Zero(),
							Duration.create(1000, MILLISECONDS), instance,
		...
		public void onReceive(Object message) {
			...
			if ("TICK".equals(message)) {
				...
				List<EventSource> shallowCopy = new ArrayList<EventSource>(
						sockets); // prevent ConcurrentModificationException
